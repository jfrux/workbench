...
const buildFeedUrl = (canary, currentVersion) => {
  ...
  return `https://${updatePrefix}.opc.ai/workbench/update/${isLinux ? 'deb' : platform}/${currentVersion}`;
};
...
module.exports = win => {
  ...
  const onupdate = (ev, releaseNotes, releaseName, date, updateUrl, onQuitAndInstall) => {
    const releaseUrl = updateUrl || `https://github.com/openpilot-community/workbench/releases/tag/${releaseName}`;
