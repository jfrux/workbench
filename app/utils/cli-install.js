...
const addSymlink = () => {
  return checkInstall().then(isInstalled => {
    if (isInstalled) {
      ...
      console.log('Workbench CLI already in PATH');
      return Promise.resolve();
    }
    //eslint-disable-next-line no-console
    console.log('Linking WorkbenchCLI');
...
const addBinToUserPath = () => {
  ...
  return new Promise((resolve, reject) => {
    ...
    envKey.values((err, items) => {
      ...
      // C:\Users\<user>\AppData\Local\workbench\app-<version>\resources\bin
      const binPath = path.dirname(cliScriptPath);
      // C:\Users\<user>\AppData\Local\workbench
      ...
      //eslint-disable-next-line no-console
      console.log('Adding WorkbenchCLI path (registry)');
...
exports.installCLI = withNotification => {
  if (process.platform === 'win32') {
    addBinToUserPath()
      ...
      .catch(err =>
        logNotify(withNotification, 'Workbench CLI installation failed', `Failed to add Workbench CLI path to user PATH ${err}`)
      );
  } else if (process.platform === 'darwin') {
    addSymlink()
      .then(() => logNotify(withNotification, 'Workbench CLI installed', `Symlink created at ${cliLinkPath}`))
      .catch(err => {
        ...
        //eslint-disable-next-line no-console
        console.error(err);
        logNotify(withNotification, 'Workbench CLI installation failed', error);
      });
  } else {
    withNotification &&
      notify('Workbench CLI installation', 'Command is added in PATH only at package installation. Please reinstall.');
  }
};
