...
const checkConfig = () => {
  ...
  msg += 'Please launch Workbench and retry.';
  console.error(msg);
  process.exit(1);
};
...
const lsRemote = pattern => {
  ...
  const URL = `https://api.npms.io/v2/search?q=${(pattern && `${pattern}+`) || ''}keywords:workbench-plugin,workbench-theme`;
  return got(URL)
  ...
  commandPromise = lsRemote(query)
    .then(entries => {
      if (entries.length === 0) {
        ...
        console.error(`${chalk.red('Try')} ${chalk.green('workbench ls-remote')}`);
        process.exit(1);
      } else {
...
args.command(['version'], 'Show the version of workbench', () => {
  console.log(version);
  process.exit(0);
});

args.command(['<default>'], 'Launch Workbench');
...
const main = argv => {
  ...
  const env = Object.assign({}, process.env, {
    // this will signal Workbench that it was spawned from this module
    HYPER_CLI: '1',
  ...
  if (!flags.verbose) {
    ...
    if (process.platform === 'darwin') {
      //Use `open` to prevent multiple Workbench process
      const cmd = `open -b co.openpilot-community.workbench ${args_}`;
      const opts = {
        env
      };
