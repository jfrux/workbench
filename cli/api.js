...
const devConfigFileName = path.join(__dirname, `../.workbench.js`);

let fileName =
  process.env.NODE_ENV !== 'production' && fs.existsSync(devConfigFileName)
    ? devConfigFileName
    : `${os.homedir()}/.workbench.js`;

/**
 * We need to make sure the file reading and parsing is lazy so that failure to
 * statically analyze the workbench configuration isn't fatal for all kinds of
