...
// https://github.com/openpilot-community/workbench/issues/619
...
Module._load = function _load(path) {
  ...
  switch (path) {
    ...
    case 'workbench/component':
      //eslint-disable-next-line no-console
      console.warn(
        'DEPRECATED: If your plugin requires `workbench/component`, it must requires `react.PureComponent` instead and bundle `react` as a dependency'
      );
      return PureComponent;
    case 'workbench/notify':
      return notify;
    case 'workbench/Notification':
      return Notification;
    case 'workbench/decorate':
      return decorate;
    default:
...
const plugins = remote.require('../app/plugins');
...
const loadModules = () => {
  ...
  modules = paths.plugins
    ...
    .map(path => {
      ...
      // mapWorkbenchTermState mapping for backwards compatibility with workbenchterm
      if (mod.mapWorkbenchTermState) {
        mod.mapWorkbenchState = mod.mapWorkbenchTermState;
        //eslint-disable-next-line no-console
        console.error('mapWorkbenchTermState is deprecated. Use mapWorkbenchState instead.');
      }

      // mapWorkbenchTermDispatch mapping for backwards compatibility with workbenchterm
      if (mod.mapWorkbenchTermDispatch) {
        mod.mapWorkbenchDispatch = mod.mapWorkbenchTermDispatch;
        //eslint-disable-next-line no-console
        console.error('mapWorkbenchTermDispatch is deprecated. Use mapWorkbenchDispatch instead.');
      }
      ...
      if (mod.mapWorkbenchState) {
        connectors.Workbench.state.push(mod.mapWorkbenchState);
      }

      if (mod.mapWorkbenchDispatch) {
        connectors.Workbench.dispatch.push(mod.mapWorkbenchDispatch);
      }
