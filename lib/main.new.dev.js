if (['--help', '-v', '--version'].includes(process.argv[1])) {
  ...
  const configLocation = process.platform === 'win32' ? process.env.userprofile + '\\.workbench.js' : '~/.workbench.js';
  //eslint-disable-next-line no-console
  console.log(`Workbench version ${version}`);
  //eslint-disable-next-line no-console
  console.log('Workbench does not accept any command line arguments. Please modify the config file instead.');
  //eslint-disable-next-line no-console
  console.log(`Workbench configuration file located at: ${configLocation}`);
...
if (process.platform === 'win32') {
  ...
  const systemContextMenu = require('../app/system-context-menu');
...
// NO PLUGINS YET OR EVER MAYBE
const plugins = require('../app/plugins');

const {installCLI} = require('../app/utils/cli-install');
const AppMenu = require('../app/menus/menu');
const Window = require('../app/ui/window');
const windowUtils = require('../app/utils/window-utils');
...
const url = 'file://' + resolve(isDev ? __dirname : app.getAppPath(), 'app.html');
//eslint-disable-next-line no-console
    ...
    .then(() => {
      ...
      if (!isDev) {
        ...
        if (config.getConfig().defaultSSHApp && !app.isDefaultProtocolClient('ssh')) {
          //eslint-disable-next-line no-console
          console.log('Setting Workbench as default client for ssh:// protocol');
          app.setAsDefaultProtocolClient('ssh');
        } else if (!config.getConfig().defaultSSHApp && app.isDefaultProtocolClient('ssh')) {
          //eslint-disable-next-line no-console
          console.log('Removing Workbench from default client for ssh:// protocol');
